    ...
    if (typeof LM.availability !== "function") {
      if (typeof LM.capabilities === "function") {
        try {
          ...
          const st = normalizeAvailability(cap?.available);
          return { status: st, api: true };
        } catch {
          /* ignore */
        }
      }
...
async function samplingParams(LM) {
  try {
    if (typeof LM.params !== "function") return { temperature: 0.0 };
...
/**
 * Create a LanguageModel session with concurrency protection and safe download tracking.
 * Triggers model download when status is downloadable / downloading.
...
export async function getLanguageModel(onProgress) {
  ...
  const createPromise = (async () => {
    ...
    try {
      ...
      if (typeof LM.availability === "function") {
        ...
        try {
          const cap = await LM.capabilities();
          status = normalizeAvailability(cap?.available);
        } catch {
          /* ignore */
        }
      }
      ...
      // We separate attempts to support different versions of Chrome Prompt API:
      // Note: Do NOT combine initialPrompts with systemPrompt to prevent TypeError / state collisions.
      const attempts = [
        { ...sample, monitor },
      ...
      // If initial attempts failed with InvalidStateError, do one final guarded retry
      if (!session && lastErr?.name === "InvalidStateError") {
        try {
          await new Promise((r) => setTimeout(r, 500));
          session = await LM.create({ monitor });
        } catch {
          /* ignore */
        }
      }
  ...
  inFlightSessionPromise = createPromise;
  try {
...
export async function ensureNano(onEvent) {
  ...
  if (session) {
    // Retain this session so the immediately following inference call doesn't need to re-create
    warmSession = session;
    emit("available", 1);
    return { ok: true, status: "available" };
  }
...
export function applyJson(doc, data) {
  ...
  if (Array.isArray(data.addresses) && data.addresses.length) {
    rec.addresses = data.addresses.map((a) => String(a).trim()).filter(Boolean);
    if (!rec.fields.address.value && rec.addresses.length) {
      set("address", rec.addresses[0], FIELD_KINDS.address);
    }
  }

  if (Array.isArray(data.phone_numbers) && data.phone_numbers.length) {
    rec.phone_numbers = data.phone_numbers.map((p) => String(p).trim()).filter(Boolean);
    if (!rec.fields.phone.value && rec.phone_numbers.length) {
      set("phone", rec.phone_numbers[0], FIELD_KINDS.phone);
    }
  }
...
async function callPrompt(session, prompt) {
  ...
  try {
    ...
    // Fallback for older message-array signature
    try {
      return await session.prompt([{ role: "user", content: prompt }]);
    } catch {
      throw err;
    }
  }
...
async function inferOne(doc, session) {
  ...
  try {
    const raw = await callPrompt(session, prompt);
    ...
    if (lastError === "QuotaExceededError") {
      try {
        ...
        lastError = err2?.name || err2?.message || lastError;
      }
    }
  }
