...
const ROLE_HINTS =
  /\b(chief|ceo|cto|cfo|coo|chairman|chairwoman|president|founder|director|vp|vice president|head of|managing director|board|officer|vorstand|vorsitzende[rn]?|geschäftsführer(?:in)?|geschäftsführung|代表取締役|取締役|執行役員|社長)\b/i;
...
export function extractFromMarkdown(doc) {
  ...
  rec.addresses = deduplicateAddresses([], allAddresses);
  if (allAddresses.length) {
  ...
  if (email) {
    ...
    rec.emails = aggregateEmails([], [email]);
  } else {
    rec.emails = [];
  }
  ...
  rec.phone_numbers = deduplicatePhones([], allPhones);
  if (allPhones.length) {
  ...
  rec.executives = (extractExecs(doc) || []).map(sanitizeExecutive).filter(Boolean);
...
function looksLikePersonName(s) {
  ...
  if (RECRUITING_PHRASES_RE.test(t)) return false;
...
function extractExecs(doc) {
  ...
  const push = (rawName, rawRole, rawEmail) => {
    ...
    const sanitized = sanitizeExecutive({ name, role });
    if (!sanitized) return;
    ...
    out.push({
      ...
      xing: "",
      wechat: "",
      confidence: ROLE_HINTS.test(sanitized.role) ? "high" : "medium",
      sourceUrl: doc.url,
      dirty: false,
      verified: false,
    });
...
export function scoreLinks(links, origin, missingKeys = []) {
  const seen = new Set();
  const scored = [];
  for (const link of links || []) {
    let abs = link.href;
    const canon = canonicalizeUrl(link.href, origin);
    if (!canon) {
      try {
    ...
    seen.add(abs);
    const hay = `${link.text || ""} ${link.href || ""} ${abs}`;

    // Trap avoidance: generic footer boilerplate, marketing blogs, auth, cart, asset extensions
    if (TRAP_RE.test(abs) || TRAP_TEXT_RE.test(hay) || PATH_TRAP_RE.test(abs)) {
      scored.push({
        href: abs,
        text: link.text,
        score: 0.01,
    ...
    // Dynamic Deficit Weighting if missing fields are specified
    if (missingKeys && missingKeys.length > 0) {
      const frontierScore = scoreFrontierLink(link, missingKeys, origin);
      if (frontierScore > best.score) {
        best = {
          href: abs,
          text: link.text,
          score: frontierScore,
          reason: `frontier · deficit (${frontierScore})`,
        };
      }
    }
  ...
  return JSON.stringify(
    {
      ...
      email: val("email"),
      emails: (rec.emails && rec.emails.length) ? rec.emails : (val("email") ? [{ email: val("email"), department: "general" }] : []),
      phone: val("phone"),
